...
| $\text{Cantidad de}$ $b's$ $\text{: }$ $m$ | $\text{Cantidad de}$ $a's$ $\text{: }$ $n$   | $\text{Posibles cadenas en }$ $L$ |
|---|-----|-----------------------|
| $0$ | $\geq 3$ | $aaa, aaaa, aaaaa, aaaaaa, ... $|
| $1$ | $\geq 4$ | $aaaab, aaaaab, aaaaaab, aaaaaaab, ...$ |
| $2$ | $\geq 5$ | $aaaaabb, aaaaaabb, aaaaaaabb, aaaaaaaabb, ...$ |

Cadenas de solo ${a’s}$ (con al menos 3)

Cadenas que empiezan con ${aaa}$ y luego por cada ${b}$ se tiene una ${a}$ para emparejar. Pueden haber más ${a’s}$ que ${b’s}$.
...
1. Verificar que la cadena empieza con 3 ${a’s}$:

${\Delta(q_{0}, a, Z_{0}) =}$ { ${(q_{1}, Z_{0})}$ }

${\Delta(q_{1}, a, Z_{0}) =}$ { ${(q_{2}, Z_{0})}$ }

${\Delta(q_{2}, a, Z_{0}) =}$ { ${(q_{3}, Z_{0})}$ }

Los estados ${q_{0}, q_{1}, q_{2}}$ verifican que la cadena empieza con 3 ${a’s}$

2. Acumular ${A's}$ en la pila a medida que se leen ${a’s}$ en la cinta.

${\Delta(q_{3}, a, Z_{0}) =}$ { ${(q_{3}, AZ_{0})}$ }

${\Delta(q_{3}, a, A) = }$ { ${(q_{3}, AA)}$ }

3. La cadena de entrada puede tener solo ${a's}$

${\Delta(q_{3}, \lambda, A) =}$ { ${(q_{f}, A)}$ }

${\Delta(q_{3}, \lambda, Z_{0}) =}$ { ${(q_{f}, A)}$ }

4. Emparejar ${b’s}$ de la cinta con ${A’s}$ de la pila:

${\Delta(q_{3}, b, A) =}$ { ${(q_{4}, \lambda)}$ }

${\Delta(q_{4}, b, A) =}$ { ${(q_{4}, \lambda)}$ }

5. Aceptar la cadena:

${\Delta(q_{4}, \lambda, A) =}$ { ${(q_{f}, A)}$ }

${\Delta(q_{4}, \lambda, Z_{0}) =}$ { ${(q_{f}, Z_{0})}$ }

Procesamiento de la cadena ${w=aaaaaabb}$

$(q_0, aaaaaabb, Z_0) \vdash
(q_1, aaaaabb, Z_0) \vdash
(q_2, aaaabb, Z_0) \vdash
(q_3, aaabb, Z_0) \vdash
(q_3, aabb, AZ_0) \vdash
(q_3, abb, AAZ_0) \vdash
(q_3, bb, AAAZ_0) \vdash
(q_4, b, AAZ_0) \vdash$
$(q_4, \lambda, AZ_0) \vdash
(q_f, \lambda, AZ_0)$

${(q_f, \lambda, AZ_0)}$ es configuración de aceptación, por lo tanto la cadena ${w=aaaaaabb}$ es aceptada por el autómata.
